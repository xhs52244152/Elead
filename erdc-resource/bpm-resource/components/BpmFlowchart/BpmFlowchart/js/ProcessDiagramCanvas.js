...
var TASK_STROKE_COLOR = "#949a94"; /*Color.SlateGrey; */
...
var HIGHLIGHT_COLOR = Color.DodgerBlue3;
var HIGHLIGHT_COLOR5 = Color.DodgerBlue3;
...
var START_EVENT_STROKE_COLOR = Color.black; /* Color.SlateGrey; 流程开始图标颜色*/
...
var MAILTASK_IMAGE = 	"images/deployer/mail.png";
...
// var DefaultProcessDiagramCanvas = {
	...
	*/
	...
	_drawFlow: function(waypoints, conditional, isDefault, highLighted, withArrowHead, connectionType){
		...
		this.setPaint(SEQUENCEFLOW_COLOR); //没有走过的线颜色
...
// TODO: generate polylineId or do something!!
		...
		window.processId = this.processDefinitionId; // 完成时获取流程定义id
	...
	*/
	...
	_drawTask: function(name, x, y, width, height, thickBorder) {
        ...
        //thickBorder  有值代表已经走过活动节点， 已经走过用灰色表示
        if (thickBorder) { // 加颜色或者加粗 边框
            this.setPaint(TASK_STROKE_COLOR);//灰色
        } else {
            this.setPaint(TASK_COLOR); //没有走的节点粉红色
        }
		// anti smoothing
		if (this.strokeWidth%2 == 1)
			x = Math.round(x) + .5, y = Math.round(y) + .5;
		...
		// shape
		var shape = this.g.rect(x, y, width, height);
	...
	*/
	...
	//活动节点画图  wencz  文传志
	drawUserTask: function(activityImpl) {
        var name = activityImpl.getProperty("name");
        var x = activityImpl.getX();
        var y = activityImpl.getY();
        var width = activityImpl.getWidth();
        var height = activityImpl.getHeight();
        var id = activityImpl.pr

        var taskComments = activityImpl.properties.taskComments;
        var activityId = null ; //看是否有id ，如果有id就代码改活动已经完成
	    ...
	    this.g.setStart();
		this._drawTask(name, x, y, width, height,activityId);
	...
	drawScriptTask: function(name, x, y, width, height) {
	  	this.g.setStart();
		this._drawTask(name, x, y, width, height,null);
	...
	drawMailTask: function(name, x, y, width, height) {
		this.g.setStart();
	  this._drawTask(name, x, y, width, height,null);
	  var img = this.g.image(MAILTASK_IMAGE, x + ICON_PADDING, y + ICON_PADDING, ICON_SIZE, ICON_SIZE);
	  var set = this.g.setFinish();
	  this.addHandlers(set, x, y, width, height, "task");
  },
	...
	*/
	...
	highLightActivity: function(activityId){
		...
		shape.attr("stroke-width", THICK_TASK_BORDER_STROKE);
		shape.attr("stroke", HIGHLIGHT_COLOR5);
        shape.attr("fill", HIGHLIGHT_COLOR5);
	},
	...
	highLightFlow: function(flowId){
		...
		st.attr("stroke", HIGHLIGHT_COLOR);  //当前节点线的颜色
		var withArrowHead = shapeFlow.data("withArrowHead");
		if (withArrowHead)
			st[1].attr("fill", HIGHLIGHT_COLOR); //貌似没什么用。
	...
	_drawMultilineText: function(text, x, y, boxWidth, boxHeight, verticalAlign, horizontalAlign) {
		...
		measurer.rafaelTextObject.attr({"y": bb.y2});
		//var bb = measurer.rafaelTextObject.getBBox();
