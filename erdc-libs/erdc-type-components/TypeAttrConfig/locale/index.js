 ...
 * **/
 define([], function () {
    ...
    const languageObj = {
        ...
        '组件配置': { CN: '组件配置', EN: 'Component Configurations' },
        ...
        '类型': { CN: '类型', EN: 'Type' },
        '选择全局属性': { CN: '选择全局属性', EN: 'Choose global properties' },
        ...
        'internalNameError1': { CN: '内部名称格式错误：如果有“.”、“-”、“_”，请将其放到中间', EN: 'Internal name format error: if there is ".", "-" or "_", please put it in the middle' },
        'internalNameError2': { CN: '内部名称格式错误：请输入字母、数字、“-”、“_”或“.”', EN: 'Internal name format error: Please enter letters, numbers, "-", "_" or "."' },
        // 校验报错
        ...
        '默认值': { CN: '默认值', EN: 'Default data' },
        virtualAttrTip: { CN: '"虚拟属性"不能使用"选择全局属性"', EN: '"Virtual Properties" cannot be used "Select Global Properties"' },
        relationAttrTip: { CN: '"关系属性"不能使用"选择全局属性"', EN: '"Relational Properties" cannot be used "Select Global Properties"' },
        maxLengthTypeTip: { CN: '数据类型为字符串时须填写最大长度，最大长度小于等于500时，数据库字段将使用varchar,否则将使用text.  <br/> 数据库字段类型为varchar类型时可以扩展长度为text，确定为text后不能再改回varchar', EN: 'The maximum length must be filled in when the data type is a string. If the maximum length is less than or equal to 500, the database field will use varchar, otherwise text will be used. <br/>  The length can be extended to text when the database field type is varchar, and cannot be changed back to varchar after determining text' },
        maxLengthChangeTip: { CN: '最大长度调小，不会修改数据库字段长度', EN: 'The maximum length is reduced, and the database field length is not modified.' },
    }
