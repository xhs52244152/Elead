...
const TreeUtil = {
     ...
     * @param { string } [options.parentField = 'parentId'] - 父节点所在字段
     * @param { string } [options.childrenField = 'children'] - 构建后子节点所在字段
     * @param { function(node: Object): boolean } [options.isRoot = (node) => ['', false, null, undefined, -1, '-1'].includes(node[parentField])] - 判断某个节点是根节点的依据
     * @param { function(node: Object, parentNode: Object): boolean } [options.isChildOf = (children, parent) => children[parentField] === parent.id] - 判断某个节点是另一个节点的子节点的依据
     * @param { function(node: Object, parentNode: Object, treeArray: Object[], nodePath: Object[]) } [options.every] - 针对每个节点的处理
     * @returns { array } - 最终树结构
     */
    buildTree: function (
        array,
        {
            ...
            isChildOf = (children, parent) => children[parentField] === parent.id,
            every = () => {
                // do nothing
            }
        } = {}
     ...
     * @param { string } [options.childrenField = 'children'] -  子节点所在字段
     * @param { function(node: Object, tree: Object[]) } [options.every = () => {}] - 每节点处理方法，传入当前节点、树结构为参数
     * @param { function(tree: Object[])  } [options.done = () => {}] - 遍历结束回调，传入树结构为参数
     */
     ...
     * @param { Object } [options]
     * @param { string } [options.childrenField = 'children'] -  子节点所在字段
     * @param { Object | function(node): boolean } [options.target = () => false] - 目标节点识别方式，如 {id: '1.1'}, node => node.id === '1.1'
     * @param { boolean } [options.isSome=true] (当children传入方式是map时)是否全量对比。即是否比对完children的键值对。
     * @return { array } - 查找到的路径
     */
     ...
     * @param { Object } [options]
     * @param { string } [options.childrenField = 'children'] - 子节点所在字段
     * @param { Object | function(node): boolean } [options.target] - 子节点识别方式，可以是对象按字段比对，也可以一个返回布尔值的函数
     * @param { boolean } [options.isSome = true] 当target是对象时使用，表示是否全量对比。即是否比对完children的键值对。
     * @return { Object }
     */
     ...
     * @param { Object } [options]
     * @param { string } [options.childrenField = 'children'] - 子节点所在字段
     * @param { node: Object, tree: Object[] } [options.every] - 每节点处理方法，传入当前节点、树结构为参数
     * @param { function(layer: Object[], tree: Object[]) } [options.doneLayer] - 每层处理方法，传入当前层节点，树结构为参数
     * @param { function(tree: Object[]) } [options.done] - 遍历结束回调，传入树结构为参数
     ...
     * @param { { children: string, attrs: string } } [opt] 树表格搜索配置，其中attrs必须是表格所显示的字段，这个字段接口返回的数据不能为对象
     * @returns
     */
