define([
    'text!' + ELMP.resource('ppm-component/ppm-components/BatchSetValue/components/SetValue/index.html'),
    ELMP.resource('ppm-component/ppm-components/BatchSetValue/customProps.js'),
    'css!' + ELMP.resource('ppm-component/ppm-components/BatchSetValue/components/SetValue/index.css')
    ...
    return {
        ...
        methods: {
            ...
            // 获取更新的数据
            getAttrData() {
                const tableData = _.map(this.tableData, (item) => {
                    return {
                        attrName: item?.attrName,
                        value: _.isObject(item?.value) ? item?.value?.value : item?.value || ''
                    };
                });
                return _.map(tableData, (item) => _.extend({}, ErdcKit.deepClone(item)));
            },
            customSchema(schema) {
                let config = ErdcKit.deepClone(schema);
                if (
                    schema.component === 'ComplexSelect' &&
                    schema.leftRequestConfig.url === '/fam/team/getTeamRoleByContainer'
                ) {
                    console.log('');
                }
                return config;
