define(['vue', 'axios', 'erdcloud.kit'], function (Vue, axios, erdcloudKit) {
    ...
    const defaultInterceptors = {
        request: {
            timestamp: function (config) {
                config.params = {
                    _t: `${new Date().getTime()}_${erdcloudKit.randomString(5)}`,
                    ...config.params
                };
                return config;
            },
            normalize: function (config) {
                ...
                if (compatMethod.includes(method)) {
            ...
            globalErrorAbortController: function (config) {
                config.transformResponse = config.transformResponse?.filter(Boolean);
                ...
                return new Promise((resolve, reject) => {
                    ...
                    if (
                        ...
                        require(['erdcloud.i18n'], function (erdcloudI18n) {
                            ...
                            let msg = backEndMsg || erdcloudI18n.translate(status);
    ...
    axios.interceptors.request.use(defaultInterceptors.request.timestamp);
    axiosInstance.interceptors.request.use(defaultInterceptors.request.normalize);
    axios.interceptors.request.use(defaultInterceptors.request.normalize);
