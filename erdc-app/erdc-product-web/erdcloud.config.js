...
const microApps = fs.readdirSync(path.join(__dirname, '../'));

const collectFolder = microApps.reduce((acc, cur) => {
    if (cur === 'erdc-product-web') {
        return acc;
    }
    return {
        ...acc,
        [`erdc-app/${cur}`]: [`../${cur}/**`, `!../${cur}/node_modules/**`]
    };
}, {
    '/erdc-libs': ['../../packages/erdc-libs/**']
});

const cbbPath = path.join(__dirname, '../../packages/erdcloud-cbb-frontend');
const cbb = fs.readdirSync(cbbPath)?.filter(item => fs.statSync(path.join(cbbPath, item)).isDirectory());
cbb.forEach(cbb => {
    collectFolder[`erdc-resource/${cbb}`] = [`../../packages/erdcloud-cbb-frontend/${cbb}/**`];
});
