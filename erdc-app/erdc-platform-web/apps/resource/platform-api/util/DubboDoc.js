define([
    ...
    class dubboDoc extends ApiDoc {
        setDefinitionOtherType(swud, value) {
            swud.type = value.javaType;
        }

        setPropertyOtherType(spropObj, propobj) {
            spropObj.validations = propobj.validations ?? [];
            spropObj.type = propobj.javaType ?? 'string';
            spropObj.description = propobj.displayName ?? '';
        ...
        parseApiTags(menu) {
            const tags = menu.tags.map((tag) => {
                return new Swagger.SwaggerBootstrapUiTag(tag.name, tag.description, tag.className, tag.deprecated);
            });
