define([
    'text!' + ELMP.resource('project-dashboard/components/IssueReport/index.html'),
        ...
        methods: {
            ...
            getEchartsOption(echartsType, conditionDtoList) {
                return this.$famHttp({
                    ...
                    method: 'POST',
                    data: {
                        typeName: 'erd.cloud.ppm.issue.entity.Issue',
                        echartsType,
                        pageSearchDto: {
            ...
            getStatusList() {
                this.$famHttp({
                    ...
                    params: {
                        ...
                        showAll: false,
                        targetClass: 'erd.cloud.ppm.issue.entity.Issue',
                        isGetDraft: false
                    }
                }).then((res) => {
            ...
            async drawBar() {
                ...
                if (document.getElementById('issueBarChart')) {
                    const myChart = echarts.init(document.getElementById('issueBarChart'));
                    // 组装每列对应的颜色标识数据
                    let colorIndexList = [];
                    data?.dataset?.source.forEach((item, index) => {
                        colorIndexList.push({
                            type: item['erd.cloud.ppm.issue.entity.Issue'],
                            colorIndex: index
                        });
                    });
                    const setOption = {
                        title: [
                            {
                                text: this.i18n.issueDistribution,
                                textStyle: {
                                    fontSize: 16,
                                    color: 'black'
                                },
                            ...
                            itemStyle: {
                                color: ({ data }) => {
                                    let colorIndex = colorIndexList?.find(
                                        (item) => item.type === data['erd.cloud.ppm.issue.entity.Issue']
                                    ).colorIndex;
            ...
            async drawPie() {
                const { data } = await this.getEchartsOption('PIE', [
                    ...
                    {
                        attrName: 'erd.cloud.ppm.issue.entity.Issue#lifecycleStatus.status',
                        oper: 'IN',
                        value1: this.selectedStatus.join(',')
                    }
                ]);
                // 基于准备好的dom，初始化echarts实例
                if (document.getElementById('issuePieChart')) {
                    const myChart = echarts.init(document.getElementById('issuePieChart'));
                    // 绘制图表
                    myChart.setOption({
                        title: [
                            {
                                text: this.i18n.totalIssues,
