define([
    'text!' + ELMP.resource('supplier-components/SupplierLinkMan/index.html'),
    'css!' + ELMP.resource('supplier-components/SupplierLinkMan/index.css'),
    'vuex',
    'erdc-kit',
    'underscore'
], function (template) {
    ...
    return {
        ...
        components: {
            ...
            SupplierAddContact: ErdcKit.asyncComponent(ELMP.resource('supplier-components/SupplierAddContact/index.js'))
        },
        data() {
            return {
                // 国际化locale文件地址
                i18nLocalePath: ELMP.resource('supplier-components/SupplierLinkMan/locale/index.js'),
                // 国际化页面引用对象
                i18nMappingObj: this.getI18nKeys(['请输入搜索关键词', '确定', '取消']),
                // 高级表格默认参数
                defaultParams: {},
                // 是否是列表
                isList: true,
        ...
        computed: {
            ...
            // 供应商视图映射
            viewTableMapping() {
                return this.getViewTableMapping({ tableName: 'linkMan' });
            },
            // 视图表格配置
            viewTableConfig() {
                return {
